...
allowed-tools: Bash(gh pr view:*), Bash(gh pr checks:*), Bash(gh pr diff:*), Bash(gh pr comment:*), Bash(gh api:*), Bash(git log:*), Bash(git blame:*), Bash(git push:*), Bash(git commit:*), Bash(git add:*), Bash(bun:*), Bash(./dist/zazu:*), Read, Write, Edit, Glob, Grep, Agent
---
...
## Phase 1: Inventory unresolved threads

Use GraphQL — REST doesn't surface thread resolution state:
...
Optionally mark the thread resolved:
...
Re-run the GraphQL query from Phase 1 — the result list should be empty (or contain only threads you've decided to defer with a documented reason).
